...
This sim was scaffolded by `npm run scaffold-screens` into `src/simulation/`, `src/sensor/` (fleet naming:
...
1. Copies the `simulation/` prototype into `src/<kebab>/` per screen
...
4. Removes the prototype `simulation/` folder
...
main.ts
  └─ SimulationScreen            (Screen<SimulationModel, SimulationScreenView>)
       ├─ SimulationModel         owns all state
       └─ SimulationScreenView    owns all visuals
```
...
template's `SimulationScreen.ts`. Screen icons live in one shared module under
...
```
src/common/MotionSensorScreenIcons.ts
```
